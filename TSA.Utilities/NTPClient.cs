...
namespace TSA.Utilities
{
    ...
    /// 
    /// LocalClockOffset - The offset of the local clock relative to the primary reference source.
    /// 
    ...
    public class NTPClient
    {
        ...
        public async Task Connect(bool UpdateSystemTime)
        {
            try
            {
                ...
                await TimeSocket.SendAsync(NTPData, NTPData.Length);
                //NTPData = TimeSocket.Receive(ref EPhost);
                UdpReceiveResult result = await TimeSocket.ReceiveAsync().ConfigureAwait(false);
        ...
        public void SyncConnect(bool UpdateSystemTime)
        {
            try
            {
                // Resolve server address
                ...
                //Connect the time server
                UdpClient TimeSocket = new UdpClient();
                TimeSocket.Client.SendTimeout = 3000;
